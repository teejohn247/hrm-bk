...
const addPermission = async (req, res) => {
    try {
        ...
        if (!moduleId || !featureId || !permissionKey || !permissionName || !permissionType) {
            return res.status(400).json({
                status: 400,
        ...
        // Get or create module document
        let moduleDoc = await Modules.findOne();
        if (!moduleDoc) {
            moduleDoc = new Modules({ modules: erpModules });
            await moduleDoc.save();
        }

        // Find module and feature
        const moduleToUpdate = moduleDoc.modules.find(m => m._id.toString() === moduleId);
        if (!moduleToUpdate) {
            return res.status(400).json({
                status: 400,
        ...
        const existingPermIndex = featureToUpdate.featurePermissions.findIndex(p => p.key === permissionKey);
        if (existingPermIndex === -1) {
            ...
            featureToUpdate.featurePermissions[existingPermIndex].name = permissionName;
            featureToUpdate.featurePermissions[existingPermIndex].permissionType = permissionType;
        }
        ...
        const updatePromises = companies.map(async (company) => {
            // Initialize companyFeatures if missing
            if (!company.companyFeatures) {
                company.companyFeatures = {
                    modules: buildModulesFromTemplate(moduleDoc.modules)
                };
            } else {
                // Sync modules with template
                company.companyFeatures.modules = syncModulesWithTemplate(
                    moduleDoc.modules,
                    company.companyFeatures.modules
                );
            }

            // Update system roles with new permission
            if (company.systemRoles && Array.isArray(company.systemRoles)) {
                updateSystemRoles(company.systemRoles, moduleToUpdate, featureToUpdate);
            }
...
// Helper function to build modules from template
function buildModulesFromTemplate(moduleTemplates) {
    return moduleTemplates.map(module => ({
        moduleId: module.moduleId,
        key: module.key,
        moduleName: module.moduleName,
        value: module.value,
        moduleFeatures: module.moduleFeatures.map(feature => ({
            ...
            featureKey: feature.featureKey,
            featureName: feature.featureName,
            featurePermissions: (feature.featurePermissions || []).map(permission => ({
                key: permission.key,
                permissionType: permission.permissionType,
                name: permission.name,
                value: false
...
// Helper function to sync modules with template while preserving existing values
function syncModulesWithTemplate(moduleTemplates, existingModules) {
    return moduleTemplates.map(moduleTemplate => {
        const existingModule = existingModules.find(m => m.moduleId === moduleTemplate.moduleId);
        ...
        const moduleFeatures = moduleTemplate.moduleFeatures.map(featureTemplate => {
            ...
            const featurePermissions = (featureTemplate.featurePermissions || []).map(permTemplate => {
                const existingPermission = existingFeature?.featurePermissions?.find(
                    p => p.key === permTemplate.key
                );
                ...
                return {
                    ...
                    permissionType: permTemplate.permissionType,
                    name: permTemplate.name,
                    value: existingPermission?.value ?? false
                };
            });
            ...
            return {
                ...
                featureName: featureTemplate.featureName,
                featurePermissions,
                _id: existingFeature?._id || new mongoose.Types.ObjectId()
            };
        });
        ...
        return {
            ...
            key: moduleTemplate.key,
            moduleName: moduleTemplate.moduleName,
            value: existingModule?.value ?? moduleTemplate.value,
            moduleFeatures
        };
    });
}

// Helper function to update system roles with new permissions
function updateSystemRoles(systemRoles, moduleToUpdate, featureToUpdate) {
    systemRoles.forEach(role => {
        if (!role.rolePermissions) return;

        const moduleInRole = role.rolePermissions.find(m => m.key === moduleToUpdate.key);
        if (!moduleInRole?.moduleFeatures) return;

        const featureInModule = moduleInRole.moduleFeatures.find(
            f => f.featureId === featureToUpdate.featureId.toString()
        );
        
        if (!featureInModule?.featurePermissions) return;

        // Add new permissions that don't exist yet
        featureToUpdate.featurePermissions.forEach(newPermission => {
            const permissionExists = featureInModule.featurePermissions.some(
                p => p.key === newPermission.key
            );
