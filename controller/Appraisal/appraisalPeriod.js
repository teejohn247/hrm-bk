...
const createPeriod = async (req, res) => {
  try {
    const { name, appraisalPeriodName, description, startDate, endDate, activeDate, inactiveDate } = req.body;
    // Support both "name" and "appraisalPeriodName" from client
    const periodName = name ?? appraisalPeriodName;
    if (!periodName || typeof periodName !== 'string' || !periodName.trim()) {
      return res.status(400).json({ status: 400, error: 'Appraisal period name is required' });
    }
    const companyId = req.payload.id;

    // Parallel fetch where possible
    const [company, existingAppraisal, appraisalGroups, employees] = await Promise.all([
      Company.findOne({ _id: companyId }),
      AppraisalGroup.findOne({ companyId, appraisalPeriodName: periodName.trim() }),
      AppraisalGrp.find({ companyId }),
      Employee.find(
        { companyId },
        { _id: 1, appraisals: 1, companyRole: 1, companyId: 1, companyName: 1,
          firstName: 1, lastName: 1, role: 1, designationName: 1, department: 1,
          fullName: 1, profilePic: 1 }
      ),
    ]);

    if (!company?.companyName) {
      return res.status(400).json({ status: 400, error: 'No company has been created for this account' });
    }

    if (existingAppraisal) {
      return res.status(400).json({ status: 400, error: 'This period name already exists' });
    }

    // Save the appraisal period
    const group = await new AppraisalGroup({
      companyId,
      companyName: company.companyName,
      appraisalPeriodName: periodName.trim(),
      description,
      startDate,
      endDate,
      activeDate,
    ...
    // Build appraisal data for each employee in parallel
    const appraisalDataDocs = await Promise.all(
      employees.map((employee) => {
        // Find KPI groups this employee is assigned to
        const kpiGroups = appraisalGroups
          .filter((grp) =>
            grp.assignedEmployees?.some(
              (emp) => String(emp.employee_id) === String(employee._id)
            )
          )
          .map(({ _id, groupName, description, groupKpis }) => ({
            groupId: _id,
            groupName,
            description,
            groupKpis,
        ...
        return new AppraisalData({
          companyId: employee.companyId,
          companyName: employee.companyName,
          payrollPeriodId: group._id,
          firstName: employee.firstName,
          lastName: employee.lastName,
          fullName: employee.fullName,
          employeeId: employee._id,
          appraisalPeriodId: group._id,
          appraisalPeriodName: periodName.trim(),
          description,
          startDate,
          endDate,
    ...
    const period = await AppraisalGroup.findOne({ _id: group._id });
    ...
    return res.status(200).json({
      ...
      data: {
        ...period.toObject(),
        appraisalData: appraisalDataDocs,
      },
    });
