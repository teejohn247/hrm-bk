...
import { parseDate, formatDateDDMMYYYY } from '../../utils/dateUtils';
const { differenceInDays, addDays, isSaturday, isSunday, isSameDay, startOfYear, endOfYear } = require('date-fns');
...
const leaveRecordsDetails = async (req, res) => {
    try {
        ...
        // Fetch company holidays for the year
        const yearStart = startOfYear(new Date(queryYear, 0, 1));
        const yearEnd = endOfYear(new Date(queryYear, 11, 31));
        ...
        console.log(`Found ${holidays.length} holidays for ${queryYear}`);
        ...
        if (allRecords.length > 0) {
            ...
            console.log("Sample record date formats:", {
                ...
                leaveEndDate: sampleRecord.leaveEndDate,
                department: sampleRecord.department,
                daysUsed: sampleRecord.daysUsed,
                daysRequested: sampleRecord.daysRequested
            });
        ...
        if (employeeIds.length > 0) {
            const employees = await Employee.find(
                { _id: { $in: employeeIds } },
                { _id: 1, profilePhoto: 1, profilePic: 1, name: 1, fullName: 1 }
            ).lean();
            
            employees.forEach(emp => {
                employeeImageMap[emp._id.toString()] = emp.profilePhoto || emp.profilePic || '';
            });
        ...
        // Step 2: Group by employee and recalculate days (excluding weekends and holidays)
        const employeeMap = {};
        ...
        recordsForYear.forEach(record => {
            ...
            if (!employeeMap[employeeId]) {
                employeeMap[employeeId] = {
                    _id: employeeId,
                    totalDaysUsed: 0,
                    totalDaysUsedRaw: 0, // Original days from records
                    totalDaysRecalculated: 0, // Days excluding weekends/holidays
                    employeeName: record.fullName,
                    department: record.department,
            ...
            // Add original days from record
            const originalDays = record.daysUsed || record.daysRequested || 0;
            employeeMap[employeeId].totalDaysUsedRaw += originalDays;
            
            // Recalculate days excluding weekends and holidays
            let recalculatedDays = originalDays; // Default to original
            ...
            try {
                if (record.leaveStartDate && record.leaveEndDate) {
                    ...
                    if (startDate && endDate && !isNaN(startDate.getTime()) && !isNaN(endDate.getTime())) {
                        // Recalculate excluding weekends and holidays
                        recalculatedDays = countWeekdays(startDate, endDate, holidays);
                        
                        console.log(`Recalculated leave for ${record.fullName}: Original=${originalDays}, Recalculated=${recalculatedDays} (excluding weekends/holidays)`);
                    }
                }
            } catch (error) {
                console.error(`Error recalculating days for record ${record._id}:`, error);
                // Fallback to original days if calculation fails
                recalculatedDays = originalDays;
            }
            
            // Add recalculated days to total
            employeeMap[employeeId].totalDaysRecalculated += recalculatedDays;
            
            // Use recalculated days as the primary metric
            employeeMap[employeeId].totalDaysUsed = employeeMap[employeeId].totalDaysRecalculated;
            
            // Add enriched record to employee's records
            const enrichedRecord = {
                ...record,
                daysUsedOriginal: originalDays,
                daysUsedRecalculated: recalculatedDays,
                excludedWeekends: originalDays - recalculatedDays > 0,
                calculationNote: recalculatedDays !== originalDays 
                    ? `Adjusted from ${originalDays} to ${recalculatedDays} days (weekends/holidays excluded)`
                    : 'No adjustment needed'
            };
            
            employeeMap[employeeId].leaveRecords.push(enrichedRecord);
        });
        ...
        const recalculationSummary = employeeTotals.map(emp => ({
            name: emp.employeeName,
            original: emp.totalDaysUsedRaw,
            recalculated: emp.totalDaysRecalculated,
            difference: emp.totalDaysUsedRaw - emp.totalDaysRecalculated
        }));
        console.log("Recalculation Summary:", recalculationSummary);
        ...
        // Group employees by day ranges (using recalculated days)
        const result = dayRanges.map(rangeInfo => {
            // Filter employees that fall within this range
            const employeesInRange = employeeTotals.filter(emp => 
                emp.totalDaysUsed >= rangeInfo.min && emp.totalDaysUsed <= rangeInfo.max
        ...
        res.status(200).json({
            ...
            calculationMethod: 'Weekends and holidays excluded from all leave calculations',
            recalculationSummary: {
                totalOriginalDays: employeeTotals.reduce((sum, emp) => sum + emp.totalDaysUsedRaw, 0),
                totalRecalculatedDays: employeeTotals.reduce((sum, emp) => sum + emp.totalDaysRecalculated, 0),
                weekendsHolidaysExcluded: employeeTotals.reduce((sum, emp) => sum + (emp.totalDaysUsedRaw - emp.totalDaysRecalculated), 0)
            }
        });
